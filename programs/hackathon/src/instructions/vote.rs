...
pub fn exec(ctx: Context<Vote>, options: String) -> Result<()> {
    ...
    let token_vote: &Account<token::Mint> = &ctx.accounts.token_vote;
    ...
    if options.len() == 0 || options.len() > proposal.max_options_per_vote {
    ...
    for option in options.into_iter() {
        ...
        proposal.num_of_nft_per_option[option] += proposal.tokens_per_option;
    }
